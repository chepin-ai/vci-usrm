...
- 判词：QF-OS 主体零私域依赖；私域额度=增益（私仓内容加工、〈RED〉 通道），非解锁条件（PURE-CI-01 律已立）。
...
| 〈RED〉 App（administration） | backup/跨域管理 | 08-20 起长期不可用→系统无碍（实证 11 天） |
...
1. **〈RED〉 通道复役**：ci-root-runner（已立 dormant）→ F7 遗物清、backup 探针、跨域管理类（administration 权限特性：解封/改设置——公仓 App 无权做）
...
- actions:write 可读运行/发 dispatch/删 secret?（secrets 需独立 secrets 权限——〈RED〉 无 secrets 权限实证：F7 之堵半因在此）
...
4. 结果经 ops-hub 令牌回写私仓（HUB-CORE 正本）+ 公域极简锚。

推论：权限面需求归零。Administration 类操作（仓库设置/secrets 普查/dispatch 授权）凡需 token 者皆走 ops-hub 或 〈RED〉（其 PEM 现仅存于 USRM-VAULT secrets，9/1 私域复通后由 ci-root-runner 自举取回）。
