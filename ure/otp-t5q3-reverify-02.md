...
| 线 | 表面态 | 根因（实证） | 阻塞归属 |
|---|---|---|---|
| qgl | 对话死（活仓 vci-qgl 仅 fleet-drive 心跳；七项 0/7） | **OTP-gate 缺 OTP_PHONE**（seq130 断代确诊；另 KIMI_SESSION_STATE stranded 于旧壳） | **EXP-043 blocked-on-root**（备件全齐，待 root 值） |
...
2. **OTP-gate 断代**：qgl 活仓 otp-gate 缺 OTP_PHONE、qlv-lib OTP×3 缺 root 值——凭证面断代非线之过（EXP-043/044 blocked-on-root）。
...
- **转发即时＝证**：field-router 01:55Z 班 success，qfa 会话包 16 件同秒 sink 201 落 ci-inbox（seq105-124）；qfa-76 实测事件延迟 2-12s（目标<60s）。
...
| wall-probe 等 cron 主搏器 | ci-control 58/vci-inbox 44 workflows | 普查提案：D-157 已改 4 器+qfa 自改 2 器，余器请 cisvr 排期事件化 |
