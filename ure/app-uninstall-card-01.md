...
## 一、为何不是我代执（证据）
卸载 installation 须以**该 App 自身 JWT** 调 `DELETE /app/installations/{id}`。六 App 之 PEM 我仅持 2（ops-hub 〈RED〉、〈RED〉 4621743，均经 vault/root 亲投）；AI-FullApp 4691638、ci-os 〈RED〉、cisbr-ci 〈RED〉 三钥不在手（E804/cisvr-57 纪律下亦不索取）→ **只有 root 能卸**，且 UI 一键比 API 更稳。
...
| ci-os | 〈RED〉 | CI-OS 遗物 | **Uninstall** |
| cisbr-ci | 〈RED〉 | CI-OS 遗物 | **Uninstall** |
