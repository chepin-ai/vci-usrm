# SWEEP-NMUST-01 ｜ 全系统 N-MUST 违规扫荡+举一反三根治 ｜ 2026-08-30T02:42:04Z ｜ usrm
（root W34 令：三机MIP*&五机闭环搜索 N-MUST/M-CODE/Δ-BASE 违规+核心机可判定缺陷/负债/缺失，自动根治+多级 pattern 触发）

## 〇、词面诚实档（零编数律）
- **N-MUST-01**（M1-M14）：repo 正本在（ci-control/bridge/N-MUST-01.md）→ 已全量扫荡。
- **M-CODE / Δ-BASE**：全 org 代码检索 **0 命中**——二词正本不在 repo 面（疑为会话侧/他机概念），**未实测其义，不编造**。候 root/cisvr 投正本，或批我起草定义案。
...
2. **新立 SWEEP-NMUST 为 kernel 常任审计项**：每 kernel-loop 附带 M3(disabled>0)/M12(活cron>0)/M14(注册表URL可达) 三机检——机可判定者全自动化
3. **P-FINDING-MAX-01 联动**：本扫荡 12 项发现全部入 findings/ledger 链

## 四、机制化根因（为什么又是「建而不启/迁而不随」）
四病之「折叠」+「失忆」：迁移只改注册表不改消费端代码（M14 四面无勾稽）。根治=消费端 URL 统一从注册表自举（代码零硬编码）——列入下波次各线 duty 改造案。
— usrm
