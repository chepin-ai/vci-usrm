#!/usr/bin/env python3
# KEY-SYNC-01 · 联邦密钥中继:满仓塔 -> 缺口仓 再密封分发
# 律:名值分离(值只经runner内存,绝不落文本/日志) · 事件驱动(零schedule) · 幂等
...
TARGETS = ['vci-aiq','vci-qlv','vci-lgt','vci-qfa','vci-qtlv','vci-control']
SYNC_SET = ['BLUEQUBIT_GMAIL','BLUEQUBIT_PW','CF_ACCOUNT_API_TOKEN','CF_ACCOUNT_ID',
...
VARS_FROM_ENV = ['SUPABASE_URL']  # 非敏感配置同步为 Variables

def gh(path, method='GET', data=None, pat=None):
    for i in range(5):
        req = urllib.request.Request('https://api.github.com'+path,
            data=json.dumps(data).encode() if data is not None else None,
            headers={'Authorization':f'token {pat or PAT}','Accept':'application/vnd.github+json',
...
def main():
    present = [n for n in SYNC_SET if os.environ.get(n)]
    print(f'[key-sync] source env present: {len(present)}/{len(SYNC_SET)} names (values masked)')
    ts = datetime.datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')
    lines = [f'# KEY-SYNC-01 receipt {ts}', '', '| name | ' + ' | '.join(TARGETS) + ' |',
             '|---|' + '---|'*len(TARGETS)]
    for name in present:
        row = [name]
        for t in TARGETS:
            row.append(str(seal(t, name, os.environ[name])))
        lines.append('| ' + ' | '.join(row) + ' |')
        print('[key-sync]', name, '->', row[1:])
    lines += ['', '## Variables']
    for vn in VARS_FROM_ENV:
        v = os.environ.get(vn)
        if not v: continue
        row = [vn] + [str(set_var(t, vn, v)) for t in TARGETS]
        lines.append('| ' + ' | '.join(row) + ' |')
        print('[key-sync][var]', vn, '->', row[1:])
    ...
    subprocess.run(['git','config','user.email','key-sync@vci-usrm'],check=True)
    ...
    for i in range(8):
        ...
        subprocess.run(['git','rebase','-q','origin/main'])
